...
- 사용자가 예약을 신청하면 대기열에서 전달받은 사용자 정보, 예약 정보, 대기 순번을 Redis에 저장
- 대기열 서버에서 5초마다 브라우저 접속 상태를 갱신 요청
- 가게 별로 관리되는 대기열에서 10초마다 최대 1,000명의 사용자를 예약 서비스로 진입시킴
- 예약 스케줄의 예약 인원을 변겨아여 예약을 선점하고 결제 진행 후 예약 확정
