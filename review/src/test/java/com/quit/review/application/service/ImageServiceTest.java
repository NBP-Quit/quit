...
class ImageServiceTest {
	...
	void create_success() throws IOException {
		...
		// when
		imageService.create(mockFile, reviewId);
	...
	@Test
	@DisplayName("리뷰를 찾을 수 없다면 CustomApiException이 발생한다.")
	void create_review_not_fount_occur_CustomApiException() throws IOException {
	    // given
		MockMultipartFile mockFile = new MockMultipartFile(
			"test",
			"test.jpg",
		...
		// when & then
		assertThatThrownBy(() -> imageService.create(mockFile, reviewId))
